...
bool visit[16][16];
int dist1[16][16],dist2[16][16],dist3[16][16],n, m, d,dx[4] = { 0,0,-1,1 }, dy[4] = { 1,-1,0,0 };
class Data {
public:
	int x, y, d;
};
		...
		for (int i = 0; i < n; i++) {
			for (int k = 0; k < m; k++) {
				...
				if (dist1[i][k] && map[i][k]) {
					...
					else if(s1.back().d == dist1[i][k]) s1.push_back(de);
				}
				if (dist2[i][k] && map[i][k]) {
					...
					else if (s2.back().d == dist2[i][k]) s2.push_back(de);
				}
				if (dist3[i][k] && map[i][k]) {
					...
					else if (s3.back().d == dist3[i][k]) s3.push_back(de);
				}
			}
		}
		if(!s1.empty()) sort(s1.begin(), s1.end(), cmp);
		if (!s2.empty()) sort(s2.begin(), s2.end(), cmp);
		if (!s3.empty()) sort(s3.begin(), s3.end(), cmp);
		if (!s1.empty() && map[s1[0].y][s1[0].x]) { map[s1[0].y][s1[0].x] = 0; kill++; }
